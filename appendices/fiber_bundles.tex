...
As we've seen in Theorem~\ref{thm:inverse}, we know some sufficient conditions in order to lift the pathogen of the inverse mapping not being continuous. We can now prove that the above example wouldn't fail if $F$ was also locally connected in addition to being Hausdorff and locally compact:
...
\begin{center}
\begin{tikzcd}
U\times F\ar[r,"\cong"']\ar[drrr,"\pi_1"',near start]\ar[rrrrrr,bend left=10,dotted,"\psi"]&U\times F_1\ar[r,"\cong"',"(f_1)_U^{-1}"]\ar[drr,"\pi_1"',near start]&p_1^{-1}(U)\ar[rr,"\phi|_{p^{-1}(U)}"]\ar[dr,"p_1"',near start]&[-2em]&p_2^{-1}(U)\ar[r,"\cong"',"(f_2)_U"]\ar[dl,"p_2",near start]&U\times F_2\ar[r,"\cong"']\ar[dll,"\pi_1",near start]&U\times F\ar[dlll,"\pi_1",near start]\\[2em]
&&&U
\end{tikzcd}
\end{center}
...
\begin{tikzcd}
U&U\times F&F\\
&U\times F\ar[ul,"\pi_1"]\ar[u,dotted,"\pi_1\times h_{\psi}^{\flat}"]\ar[ur,"h_{\psi}^{\flat}"]
\end{tikzcd}
\end{center}
