...
\begin{theorem}[Dieudonn\'e] Every paracompact space is normal.
\end{theorem}

Paracompact spaces are quite common. For example, we quote here two well known results about them. For more information refer to \cite{paracompactKelley}.
\begin{theorem}[Stone] Every metric space is paracompact.
\end{theorem}

\begin{theorem}[Morita] If a regular topological space is the countable union of compact subsets, then it is paracompact
\end{theorem}
\begin{remark}\label{rem:Gr_paracompact} For $k,n\in\mathbb{N}$ such that $k<n$, $\Gr{k}{n}$ and $\StO{k}{n}$ are paracompact.
\end{remark}
