...
At this point we answered positively the question about decomposing a vector bundle, provided that the vector bundle in question is a euclidean vector bundle. A very natural question now is how restrictive is the notion of a euclidean vector. As it is shown at the end of the next section, a vector bundle admits a euclidean vector bundle structure at least when the base space is paracompact. Most familiar topological spaces we are paracompact, as we hint in the next section.

\section{Paracompactness}


A very important property of paracompact spaces is the connection with the partitions of unity, as presented in the next well-known result. The proof of it is out of the scope of this thesis and the reader can refer to \cite{bredon}, where it is stated and proven as Theorem~12.8 and Proposition~12.9.
\begin{theorem} Let $X$ be a paracompact topological space and let $\mathcal{U}=\{U_i\}_{i\in I}$ be an open covering of $X$, then there exists some locally finite open covering $\mathcal{V}=\{V_a\}_{a\in A}$ of $X$ and a partition of unity $\{f_a:X\to[0,1]\}_{a\in A}$, such that $A\subseteq I$, $\mathrm{supp}(f_a)\subseteq V_a$ and $V_a^-\subseteq U_a$ for every $a\in A$.
\end{theorem}
