...
\item Next it is proven that for every $H\in\Gr{k}{n}$, the following subset of $\mathbb{R}^n$ is a neighborhood of $H$, homeomorphic to $\mathbb{R}^{k(n-k)}$:
...
First notice $r_0$ is well defined, since for $1\leq\ell_1<\ell_2\leq k$, we have that \[u_{\ell_2}\perp H\cap\mathbb{F}_{j_{\ell}-1}\supseteq H\cap\mathbb{F}_{j_{\ell-1}}\ni u_{\ell_1}\]
