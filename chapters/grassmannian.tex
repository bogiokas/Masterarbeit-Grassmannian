...
\[\tilde{\mathcal{C}}_{\mathbf{j}}(\mathbb{F}_{\bullet},f_{\bullet}):=\left\{(v_1,v_2,\ldots,v_k)\in\StO{k}{n}:v_{\ell}\in\mathbb{H}_{j_{\ell}}(\mathbb{F}_{\bullet},f_{\bullet})\ \forall\ell\in[k]\right\}\]
...
\[\mathbb{H}_i(\mathbb{F}_{\bullet},f_{\bullet}):=\left\{v\in\mathbb{F}_i:v^tf_i>0\right\}\]
for every $i\in[n]$.
\end{definition}
...
\[{\tilde{\mathcal{C}}_{\mathbf{j}}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}}^-=\left\{(v_1,v_2,\ldots,v_k)\in\StO{k}{n}:v_{\ell}\in{\mathbb{H}_{j_{\ell}}(\mathbb{F}_{\bullet},f_{\bullet})}^-\ \forall\ell\in[k]\right\}\]
where:
\[{\mathbb{H}_i(\mathbb{F}_{\bullet},f_{\bullet})}^-:=\left\{v\in\mathbb{F}_i:v^tf_i\geq 0\right\}\]
\end{lemma}
...
&=&{\left\{(v_1,v_2,\ldots,v_k)\in\StO{k}{n}:v_{\ell}\in\mathbb{H}_{j_{\ell}}(\mathbb{F}_{\bullet},f_{\bullet})\ \forall\ell\in[k]\right\}}^-\\
...
\begin{lemma} For any integers $0<k<n$, for any flag $\mathbb{F}_{\bullet}$ of $\mathbb{R}^n$, for any orthonormal basis $f_{\bullet}$ of $\mathbb{R}^n$, compatible with $\mathbb{F}_{\bullet}$ and for any set $\mathbf{j}\in\binom{[n]}{k}$, the projection to the first coordinate
\[\pi_1:\tilde{C}_{\mathbf{j}}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-\to S^{n-1}\cap\mathbb{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-\]
is a trivial fiber bundle.
\end{lemma}
\begin{proof} First of all, the map $\pi_1$ is well defined, since it takes $(v_1,\ldots,v_k)$ to $v_1$, which is a unit vector and also an element of $\mathbb{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-$, by Lemma~\ref{lem:closure_of_cells}. Notice that since $f_{j_1}\in\mathbf{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-$, it suffices to prove that there exists a homeomorphism $\phi$ making the following diagram commute:
\begin{center}
\begin{tikzcd}
\tilde{C}_{\mathbf{j}}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-\ar[r,"\phi","\cong"']\ar[d,"\pi_1"']&\big(S^{n-1}\cap\mathbb{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-\big)\times\pi_1^{-1}(f_{j_1})\ar[dl,"\pi_1"]\\
S^{n-1}\cap\mathbb{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-
\end{tikzcd}
\end{center}
First, we are going to define a transformation $\rho_{v}\in SO(n)$ for every $v\in S^{n-1}\cap\mathbb{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-$, which takes $v$ to $f_{j_1}$ and is the identity in the orthogonal complement of the plane spanned by $v$ and $f_{j_1}$. Next we are going to define $\phi$ and argue that it is continuous. We will then use $\rho_{v}$ to map homeomorphically $\pi_1^{-1}(v)$ to $\pi_1^{-1}(f_{j_1})$ for each $v$. Finally we will glue all of $\rho_v$ to a homeomorphism $\phi$.

As we prove in Computation~\ref{com:rotation}, for any $v\in S^{n-1}\cap\mathbb{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-\setminus\{f_{j_1}\}$, the rotation on the plane $\mathrm{span}\{v,f_{j_1}\}$ in $\mathbb{R}^n$ which takes $v$ to $f_{j_1}$ is given by:
...
\begin{center}
\begin{tikzcd}
S^{n-1}\cap\mathbb{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-\ar[r,"\rho"]&[2em]SO(n)\\[-1.5em]
v\ar[r,mapsto]&\rho_v
\end{tikzcd}
\end{center}
...
\begin{center}
\begin{tikzcd}
SO(n)\times \tilde{C}_{\mathbf{j}}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-\ar[r,hook]&SO(n)\times(\mathbb{R}^n)^k\ar[r,"\mathrm{ev}^k"]&(\mathbb{R}^n)^k\\[-1.5em]
...
since $v_{\ell}\in\mathbb{H}_{j_{\ell}}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-$. This proves that $(\rho_{v_1}v_1,\ldots,\rho_{v_1}v_k)\in\tilde{C}_{\mathbf{j}}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-$. Also, since $\rho_{v_1}v_1=f_{j_1}$, it is also true that $\pi_1(\rho_{v_1}v_1,\ldots,\rho_{v_1}v_k)=f_{j_1}$, which proves the claim.

Let us now define $\phi=\left<\pi_1,\mathrm{ev}^k\circ\left<\rho\circ\pi_1,id\right>\right>$, i.e.:
\begin{center}
\begin{tikzcd}
\tilde{C}_{\mathbf{j}}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-\ar[r, "\left<\pi_1{,}\mathrm{ev}^k\circ\left<\rho\circ\pi_1{,}id\right>\right>"]&[4em]\big(S^{n-1}\cap\mathbb{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-\big)\times\pi_1^{-1}(f_{j_1})\\[-1.5em]
(v_1,\ldots,v_k)\ar[r,mapsto]&\big(v_1,(\rho_{v_1}v_1,\ldots,\rho_{v_1}v_k)\big)
\end{tikzcd}
\end{center}
This is well defined and continuous. We will now prove that its restriction on each fiber $\pi_1^{-1}(v)$ is a homeomorphism. So, we will show that the following map is a homeomorphism for each $v\in S^{n-1}\cap\mathbb{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-$:
\begin{center}
\begin{tikzcd}
...
Since $\tilde{C}_{\mathbf{j}}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-$ is a closed subset of the compact $\StO{k}{n}$, it is also compact. The space $\big(S^{n-1}\cap\mathbb{H}_{j_1}{\left(\mathbb{F}_{\bullet},f_{\bullet}\right)}^-\big)\times\pi_1^{-1}(f_{j_1})$ is obviously Hausdorff, so using Proposition~\ref{prop:extend_fb}, we have that $\phi$ is also a homeomorphism, i.e. $\pi_1$ is a trivial fiber bundle.
\end{proof}

\begin{computation}\label{com:rotation} Let $x,y\in S^{n-1}$ be any two unit vectors in $\mathbb{R}^n$ such that $x\neq y$ and $x\neq -y$. Let $\rho_{x\to y}$ be the rotation taking $x$ to $y$ and fixing the $n-2$ dimension subspace $\mathrm{span}\{x,y\}^{\perp}$. Then:
\[\rho_{x\to y}=I_n+2yx^t-\frac{(x+y)(x+y)^t}{x^ty+1}\in SO(n)\]
\end{computation}
\begin{proof} Let $\theta\in(0,\pi)$ be the angle between $x$ and $y$ and $H=\mathrm{span}\{x,y\}$ the plane spanned by $x,y$. It is easy to compute that the following is an orthonormal basis of $H$:
...
&=-1-2\cos\theta+2\frac{(1+\cos\theta)(\cos\theta+1)}{\cos\theta+1}\\
&=1
\end{align*}
...
%
%\[\St{k}{n}\cap\cdots=
%\{(v_1,\ldots,v_k):(v_1,\ldots,v_{k-1})\in\St{k}{n}{F}\ v_{\ell}\in F_{j_{\ell}}\ v_k\cdot v_{\ell}=\delta_{k,l}\}\]
%
%
