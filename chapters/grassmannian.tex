...
%so let $(v_1,\ldots,v_k)\in\tilde{U}$. Since $q(v_1,\ldots,v_k)\in q(U)$, there exists some $(u_1,\ldots,u_k)\in U$ such that $\mathrm{span}\{v_1,\ldots,v_k\}=\mathrm{span}\{u_1,\ldots,u_k\}$. Fix now any $n-k$ vectors $c_{k+1},\ldots,c_n\in\mathbb{R}^n$ such that $\det(u_1,\ldots,u_k,c_{k+1},\ldots,c_n)\neq0$. Such a choice exists, since $u_1,\ldots,u_k$ are linearly independent. We define now the function $U\to\mathbb{R}$ taking some $(x_1,\ldots,x_k)$ to $\det(x_1,\ldots,x_k,c_{k+1},\ldots,c_n)$. This is a continuous function and thus there exists some open $A\subseteq U$, containing $(u_1,\ldots,u_k)$ such that $\det(x_1,\ldots,x_k)\neq0$ for every $(x_1,\ldots,x_k)\in A$. This means that $x_1,\ldots,x_k,c_{k+1},\ldots,c_n$ is a basis of $\mathbb{R}^n$ for every $(x_1,\ldots,x_k)\in A$. Moreover, for every $(x_1,\ldots,x_k)$ we define the linear isomorphism $\phi_{(x_1,\ldots,x_k)}:\mathbb{R}^n\to\mathbb{R}^n$ such that $\phi_{(x_1,\ldots,x_k)}(x_i)=u_i$ for every $i\in[k]$ and $\phi_{(x_1,\ldots,x_k)}(c_i)=c_i$ for every $i\in[n]\setminus[k]$. Moreover, we define the linear functions $R:\mathbb{R}^n\to\mathbb{R}^k$ and $S:\mathbb{R}^k\to\mathbb{R}^n$ such that $R(u_i)=e_i$ for every $i\in[k]$, $R(c_i)=0$ for every $i\in[n]\setminus[k]$ and $S(e_i)=u_i$ for every $i\in[k]$, where $e_1,\ldots,e_k$ is the standard basis of $\mathbb{R}^k$.





%and thus the following set of linear isomorphisms is well defined: $\Phi_{(x_1,\ldots,x_k)}:=\big\{\phi\in GL(n):\mathrm{span}\{\phi(x_1),\ldots,\phi(x_k)\}=\mathrm{span}\{x_1,\ldots,x_k\}\text{ and }\forall i\in[n]\setminus[k]\ \phi(c_i)=c_i\big\}$. We can now define:
%\[V:=\bigcup_{(x_1,\ldots,x_k)\in A}\bigcup_{\phi\in\Phi_{(x_1,\ldots,x_k)}}\phi(A)\]



%Define now $\phi:\mathbb{R}^n\to\mathbb{R}^n$ to be the linear transformation that maps $u_i$ to $v_i$ for every $i\in[k]$ and $\bar{u}_i$ to $\bar{u}_i$ for every $i\in[n]\setminus[k]$. This is well defined, since $u_1,\ldots,u_k,\bar{u}_{k+1},\ldots,\bar{u}_n$ is a basis of $\mathbb{R}^n$. Moreover, since $\mathrm{span}(v_1,\ldots,v_k)=\mathrm{span}(u_1,\ldots,u_k)$, the vectors $v_1,\ldots,v_k,\bar{u}_{k+1},\ldots,\bar{u}_n$ are also linearly independent. Hence, $\phi$ is a linear isomorphism. This means that there exists a linear isomorphism $\phi^{\oplus k}:(\mathbb{R}^n)^k\to(\mathbb{R}^n)^k$ taking $(x_1,\ldots,x_k)$ to $(\phi(x_1),\ldots,\phi(x_k))$. Thus, $\phi^{\oplus k}(A)$ is an open set containing $(v_1,\ldots,v_k)=\phi^{\oplus k}(u_1,\ldots,u_k)$.
